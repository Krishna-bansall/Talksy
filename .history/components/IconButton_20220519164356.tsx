...
interface IVariantBtnProps {
  icon: any
  isSelected: IBtnState
  id: number
  setState: React.Dispatch<React.SetStateAction<IBtnState>>
}
...
export const IconButtonVariant: React.FC<IVariantBtnProps> = ({
  ...
  let BtnState: IBtnState

  const handleClick = (): void => {
    if (isSelected.isSelected && isSelected.changeId === id) {
      BtnState = { changeId: id, isSelected: true }
    }
    setState(BtnState)
  }
    ...
    <button onClick={handleClick}>
      <div
        className={`flex h-16 w-16 cursor-pointer items-center justify-center rounded-xl ${
          isSelected.isSelected && isSelected.changeId === id
